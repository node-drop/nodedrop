...
/**
 * Service for managing workflow environments using Drizzle ORM
 * Handles deployment, promotion, rollback, and comparison of workflows across environments
 */
export class WorkflowEnvironmentServiceDrizzle {
  /**
  ...
  async getWorkflowEnvironments(
    ...
    const environments = await db.query.workflowEnvironments.findMany({
      where: eq(workflowEnvironments.workflowId, workflowId),
      orderBy: workflowEnvironments.environment,
    });
  ...
  async getEnvironment(
    ...
    const env = await db.query.workflowEnvironments.findFirst({
      where: and(
        eq(workflowEnvironments.workflowId, workflowId),
        eq(workflowEnvironments.environment, environment)
      ),
    });
  ...
  async createEnvironment(
    ...
    const existing = await db.query.workflowEnvironments.findFirst({
      where: and(
        eq(workflowEnvironments.workflowId, workflowId),
        eq(workflowEnvironments.environment, environment)
      ),
  ...
  async updateEnvironment(
    ...
    const existingEnv = await db.query.workflowEnvironments.findFirst({
      where: and(
        eq(workflowEnvironments.workflowId, workflowId),
        eq(workflowEnvironments.environment, environment)
      ),
    ...
    const [updatedEnv] = await db
      ...
      .set({
        ...
        variables: copyVariables
          ? ((workflow as any).variables || existingEnv.variables)
      ...
      .where(eq(workflowEnvironments.id, existingEnv.id))
      .returning();
  ...
  async deployToEnvironment(
    ...
    const source = await db.query.workflowEnvironments.findFirst({
      where: and(
        eq(workflowEnvironments.workflowId, workflowId),
        eq(workflowEnvironments.environment, sourceEnvironment)
      ),
    });
    ...
    const existingTarget = await db.query.workflowEnvironments.findFirst({
      where: and(
        eq(workflowEnvironments.workflowId, workflowId),
        eq(workflowEnvironments.environment, targetEnvironment)
      ),
    });
    ...
    if (existingTarget) {
      ...
      const [updated] = await db
        ...
        .where(eq(workflowEnvironments.id, existingTarget.id))
        .returning();
      targetEnv = updated;
    } else {
  ...
  async rollbackEnvironment(
    ...
    // Get deployment record with environment
    const deployment = await db.query.workflowEnvironmentDeployments.findFirst({
      where: eq(workflowEnvironmentDeployments.id, deploymentId),
    ...
    const [rolledBackEnv] = await db
      ...
      .where(eq(workflowEnvironments.id, deployment.environmentId))
      .returning();

    // Create rollback deployment record
    await db.insert(workflowEnvironmentDeployments).values({
  ...
  async getEnvironmentSummaries(
    ...
    const environments = await db.query.workflowEnvironments.findMany({
      ...
      with: {
        deployments: {
          orderBy: desc(workflowEnvironmentDeployments.deployedAt),
          limit: 1,
        },
      },
    });
    ...
    return environments.map((env) => {
      ...
      return {
        ...
        lastDeployment: lastDeployment && lastDeployment.deployedAt
          ? {
  ...
  async getDeploymentHistory(
    ...
    const [deployments, countResult] = await Promise.all([
      db.query.workflowEnvironmentDeployments.findMany({
        where: eq(workflowEnvironmentDeployments.environmentId, env.id),
        orderBy: desc(workflowEnvironmentDeployments.deployedAt),
        offset: skip,
        limit,
      }),
      db
        ...
        .from(workflowEnvironmentDeployments)
        .where(eq(workflowEnvironmentDeployments.environmentId, env.id)),
    ]);
